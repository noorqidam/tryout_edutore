  ...
  <div>
    <div v-if="show" class="col card-table">
      ...
      <div class="row row-book">
        <div v-for="item in historyQuiz" :key="item.id" class="col-6 col-lg-4 col-sm-4">
          <div class="card">
            <div class="card-body">
...
export default {
  ...
  methods: {
    async _actionGetHistoryQuiz (token) {
      await this.$axios.get('history-quiz', { headers: { 'x-access-token': token } }).then((response) => {
        const dataHistoryQuiz = response.data.data
        if (response.data.success === 'true') {
          this.historyQuiz = dataHistoryQuiz
          this.show = true
        } else {
          this.show = false
        }
