                ...
                <div style="text-align: justify;text-justify: inter-word">
                  <div v-if="ujian.description ? ujian.description.length >150 : 0">
                    <span v-if="!readMoreActivated" class="span-syarat">
                      <p v-html="ujian.description" /></span>
...
export default {
  ...
  methods: {
    ...
    _actionGetByid (id) {
      this.$axios.get('ujian-mandiri/' + id)
        .then((response) => {
    ...
    _actionStartTest () {
      ...
      if (this.ujian.type_ujian === 'BERSAMA') {
        if (this.ujian.is_private) {
          this.$router.push({
            path: '/start-ujian/bersama/access-pin/',
            params: { id: this.ujian._id },
            query: { _id: this.ujian._id, type: this.ujian.type_ujian, sesi_ujian: sesiId, slug: this.ujian.ujian_slug }
          })
        } else {
          this.$router.push({
            path: '/start-ujian/bersama/',
            params: { id: this.ujian._id },
            query: { type: this.ujian.type_ujian, _id: this.ujian._id, sesi_ujian: sesiId, name: this.ujian.slug_ujian }
          })
        }
