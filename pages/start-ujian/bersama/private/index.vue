  ...
  <div>
    ...
    <div class="row">
      ...
      >
        ...
        <div class="row justify-content-center">
          <div class="d-flex justify-content-center">
            <img src="~/assets/img/edu-wink.gif" class="img-fluid" alt="~/assets/img/edu-wink.gif" style="width: 100px;height:auto">
          </div>
        </div>
        <p class="text-center mt-2">
          Loading...
    ...
    <nav class="navbar navbar-expand-sm navbar-light bg-blues is-desktop fixed-top">
      ...
      <div v-for="(n , no) in soals" :key="n._id" style="position: fixed;right:0;margin-right:5px">
        <span v-if="current_soal === n._id" class="badge lg badge-primary p-2"> No. {{ ++no }}</span>
      </div>
      <!-- <div class="row justify-content-center timer class="text-center sub-timer"" />
    ...
    <div v-if="!loadingSoal" class="content-body">
      <div class="row flex-xl-nowrap2">
        ...
        <div class="bd-content col-md-9 col-xl-8 col-12 pb-md-3 pl-md-5">
          ...
          <div v-if="showButtonFinish" class="row justify-content-center">
...
export default {
  ...
  data () {
    return {
      ...
      jadwalsesi: {},
      next_sesi_id: null,
      next_sesi_id_name: null,
      button_waiting: false,
      FormJawab: null,
      loadingstopujian: false,
      btn_disable: false
    }
  },
  head () {
    ...
    firebase.auth().onAuthStateChanged((user) => {
      if (user) {
        ...
        user.getIdToken(true).then((accesstoken) => {
          ...
          // this.$state.$store.commit('auth/updateToken', accesstoken)
        })
      } else {
        const sesiId = this.$route.query.sesi_ujian
        this.$router.push({
  ...
  methods: {
    ...
    /** Get Question Soal Private Ujian require
     *  body @ujian_id
     */
    _actionGetQuestion (id, sesiId) {
      ...
      this.$axios.post('soal-ujian-private-in-sesi', data, { headers: { 'x-access-token': this.token } })
        .then((response) => {
            ...
            if (!response.data.soals) {
              ...
              if (moment(maxDateNTime).isBefore(timesNow)) {
                ...
                if (AllHasAnswered) {
                  ...
                  // eslint-disable-next-line no-console
                  console.log('Jumlah Sesi ' + lengthSchedule)
                  if (index === lengthSchedule) {
                    ...
                    this.next_sesi_id_name = getNextIndex.sesi_ujian
                  ...
                  // this._actionChechExsistAnswer(this.current_soal)
    ...
    _actionStoreAnswer (soalId, jawaban, next, maxIndexSoal) {
      ...
      // eslint-disable-next-line handle-callback-err
        ...
        if (error.response.status === 409) {
          ...
          // this.$store.commit('Auth/refreshToken').then((response) => {})
    ...
    _actionChechExsistAnswer (soalId) {
      this.$axios.get('check-jawaban-in-ujian?ujian_id=' + this.ujian_id + '&soal_id=' + soalId, { headers: { 'x-access-token': this.token } })
        .then((response) => {
          // eslint-disable-next-line no-console
    ...
    _actionEndUjian () {
      ...
      //     this.$router.push({
      //       path: '/start-ujian/finish',
      //       query: { ujian_id: this.ujian_id, judul_ujian: this.title }
      //     })
      //   })
      //   .catch((err) => {
      //     this.loadingstopujian = false
    ...
    _nextSesiUjian () {
      ...
      this.$router.push({
        ...
        query: { ujian_id: this.ujian_id, sesi_id: this.next_sesi_id }
      })
    },
