...
export default {
  ...
  methods: {
    _actionGetDetailUjian (id) {
      this.$axios.get('all-ujian/' + id, { headers: { 'x-access-token': this.token } })
        .then((response) => {
    ...
    _actionValidatePin () {
      ...
      this.$axios.post('verify-pin-ujian', data, { headers: { 'x-access-token': this.token } })
        .then((response) => {
          if (response.data.success === 'true') {
            this.$router.push({
              path: '/start-ujian/mandiri/private',
