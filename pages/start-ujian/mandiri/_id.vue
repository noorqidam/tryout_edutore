...
export default {
  ...
  mounted () {
    ...
    firebase.auth().onAuthStateChanged((user) => {
      if (user) {
        ...
        user.getIdToken(true).then((accesstoken) => {
          ...
          this.actionGetDaftarSesiUjian(id)
    ...
    clearInterval(this.realtimeserver)
  },
  methods: {
    _actionGetRealtimeCurrenttimeServer () {
      const socket = io('https://api-tryout.edutore.net', { 'transports': ['websocket', 'polling'] })
    ...
    async actionGetDaftarSesiUjian (id) {
      await this.$axios.get('jadwal-sesi-ujian/' + id, { headers: { 'x-access-token': this.token } })
        .then((response) => {
    ...
    _actionGetQuestion (id, sesiId) {
      ...
      this.$axios.post('soal-ujian-mandiri', data, { headers: { 'x-access-token': this.token } })
        .then((response) => {
          if (response.data.success === 'true') {
            ...
            if (!response.data.soals) {
              ...
              const getStartLastTime = response.data.start.createdAt
              this.ujian = ujian
              this.title = ujian.judul_ujian
              const timesNow = moment().format('YYYY-MM-DD HH:mm')
              let minutes = 0
              ...
              const maxDateNTime = moment(getStartLastTime).add(minutes, 'm')
    ...
    _actionEndUjian () {
      ...
      this.$axios.post('stop-ujian', data, { headers: { 'x-access-token': this.token } })
        .then((response) => {
          ...
          this.$router.push({
            path: '/start-ujian/mandiri/finish',
    ...
    _nextSesiUjian () {
      ...
      if (this.ujian.is_private) {
        this.$router.push({
          path: '/start-ujian/mandiri/access-pin/' + this.ujian_id,
          query: { _id: this.ujian._id, type: this.ujian.type_ujian, sesi_ujian: this.sesiId, slug: this.ujian.ujian_slug }
        })
      } else {
        this.$router.push({
          path: '/start-ujian/mandiri/' + this.ujian._id,
          query: { type: this.type_ujian, _id: this.ujian_id, sesi_ujian: this.sesiId, name: this.ujian.slug_ujian }
        })
      }
    },
