    ...
    <div class="container">
      <div class="row align-items-center">
        ...
        <div class="col-md-6 text-center">
          ...
          <!-- <nuxt-link :to="{ path :" class="btn btn-primary mt-5">
            Mulai
          </nuxt-link> -->
          <button @click="StartLatihan" class="btn btn-md btn-primary mt-5">
            <i class="fa fa-play" /> Mulai
          </button>
          <div class="mt-3">
...
export default {
  ...
  beforeCreate () {
    this.module_slug = this.$route.query.slug
    firebase.auth().onAuthStateChanged((user) => {
      if (user) {
        user.getIdToken(true).then((accessToken) => {
          ...
          this._actionGetCountSoals(ModuleId, paketSoalId)
  ...
  methods: {
    ...
    _actionGetCountSoals (id, psId) {
      this.isLoading = true
      this.$axios.get('/count-my-module-soal/' + id + '/paket-soal/' + psId, { headers: { 'x-access-token': this.token } })
        .then((response) => {
          if (response.data.success === 'true') {
            this.jumlah_soal = response.data.data
          }
        })
    },
    StartLatihan () {
      ...
      if (DataLocal) {
        localStorage.removeItem('test_' + this.paketSoalId)
      }
      this.$router.push({
        path: '/my-modules/start-test/' + this.ModuleId + '/' + this.paketSoalId,
        query: { subscription: this.subscription_id, module_slug: this.$route.query.slug }
      })
    }
  }
}
