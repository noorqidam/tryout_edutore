      ...
      >
        ...
        <div class="row justify-content-center">
          <div class="d-flex justify-content-center">
            <b-img src="~/assets/img/edu-wink.gif" thumbnail fluidphoto alt="Image 1" />
          </div>
        </div>
...
export default {
  ...
  beforeCreate () {
    firebase.auth().onAuthStateChanged((user) => {
      if (user) {
        ...
        this.$router.push({
          path: '/login',
          query: { quiz_id: this.$route.query._id, quiz: this.$route.query.name }
        })
      }
    })
  },
  methods: {
    ...
    async _actionAnswer (soalId, jawaban) {
      ...
      await this.$axios.post('jawab-quiz-realtime-soal/' + this.quiz_id, data, { headers: { 'x-access-token': this.token } })
        .then((response) => {
          this._functionStop()
          this.showSoal = false
          this.soals = []
